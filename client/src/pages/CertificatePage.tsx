    ...
    <div className="min-h-[calc(100vh-4rem)] bg-canvas py-6 px-4 max-w-3xl mx-auto space-y-6 print:bg-white print:text-black print:p-0">
      ...
      <div className="flex items-center justify-between print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            to={`/verify/${cert.id}`}
            className="px-4 py-2 rounded-md bg-primary hover:bg-primary-hover active:bg-primary-active text-on-primary text-xs font-semibold inline-flex items-center gap-1.5 transition-colors shadow-sm"
          >
      ...
      <div className="bg-surface-card-dark border border-hairline-dark rounded-xl p-6 sm:p-10 shadow-2xl relative overflow-hidden print:border print:border-black print:shadow-none print:bg-white">
        {/* Certificate Header */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-hairline-dark print:border-black/20">
          ...
          <div className="text-left sm:text-right">
            ...
            <span className="font-mono text-xs font-bold text-on-dark select-all">
              {cert.id}
            </span>
          </div>
        </div>
        ...
        {/* Status Seal Bar */}
        <div className="my-6 p-4 rounded-xl bg-canvas border border-hairline-dark flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:bg-transparent print:border-black/20">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-trading-up/10 text-trading-up border border-trading-up/20">
              <ShieldCheck className="w-6 h-6" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono font-bold text-trading-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4 text-xs font-mono">
          <div className="p-4 rounded-xl bg-surface-elevated-dark/40 border border-hairline-dark space-y-2.5">
            ...
            <div className="flex justify-between">
              <span className="text-muted">Sender VPA</span>
              <span className="text-on-dark select-all">{cert.senderVpa}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Receiver VPA</span>
              <span className="text-on-dark select-all">{cert.receiverVpa}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Receiver Legal Name</span>
              <span className="text-on-dark font-sans font-medium">{cert.receiverName}</span>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-surface-elevated-dark/40 border border-hairline-dark space-y-2.5">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-on-dark tnum">{new Date(cert.issuedAt).toUTCString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Geohash Anchor</span>
              <span className="text-on-dark">{cert.geohash}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Device Attestation</span>
              <span className="text-on-dark truncate max-w-[170px]">{cert.deviceAttestation}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Merkle Root</span>
              <span className="text-on-dark truncate max-w-[170px] select-all">{cert.merkleRoot}</span>
            </div>
          </div>
        </div>

        {/* Cryptographic Proof Block */}
        <div className="mt-4 p-5 rounded-xl bg-canvas border border-hairline-dark space-y-3.5">
          ...
          <div className="space-y-1">
            <div className="text-[10px] uppercase font-mono text-muted font-semibold">SPYDE Issuer Public Key</div>
            <div className="p-2.5 rounded-lg bg-surface-card-dark border border-hairline-dark text-[11px] font-mono text-muted select-all break-all">
              {cert.publicKey}
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="p-2.5 rounded-lg bg-surface-card-dark border border-hairline-dark text-[11px] font-mono text-on-dark select-all break-all flex items-center justify-between">
          ...
          <div className="space-y-1">
            ...
            <div className="p-2.5 rounded-lg bg-surface-card-dark border border-hairline-dark text-[10px] font-mono text-muted select-all break-all leading-relaxed max-h-20 overflow-y-auto">
              {cert.signature}
            </div>
          </div>
        </div>

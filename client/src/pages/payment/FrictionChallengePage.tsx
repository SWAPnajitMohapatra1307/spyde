...
function isLivenessPassed(raw?: string | null): boolean {
...
function isLivenessFailed(raw?: string | null): boolean {
  const v = (raw || '').toUpperCase();
  return v === 'FAILED' || v === 'EXPIRED' || v === 'FAIL' || v === 'REJECTED';
}
...
export const FrictionChallengePage: React.FC = () => {
  ...
  const livenessSessionId = challengeSessionId || transactionId || 'demo-session';

  const host = window.location.host;
  const protocol = window.location.protocol;
  const challengeUrl = challengeCode
    ? `${protocol}//${host}/liveness/${livenessSessionId}?code=${challengeCode}`
    : `${protocol}//${host}/liveness/${livenessSessionId}`;
  ...
  useEffect(() => {
    if (!livenessSessionId || hasHandledSuccessRef.current) return;
    setStep('AWAITING_RECEIVER');
    ...
    const interval = setInterval(async () => {
      ...
      try {
        const res = await apiClient.get<any>(
          `/api/liveness/status/${livenessSessionId}`
        );
        ...
        if (isLivenessPassed(currentStatus) && !hasHandledSuccessRef.current) {
          ...
          isLivenessFailed(currentStatus) &&
  ...
  }, [livenessSessionId, setStep, navigate]);
    ...
    <div className="min-h-[85vh] flex flex-col items-center justify-center p-4 max-w-md mx-auto space-y-6">
      <div className="w-full bg-surface-card-dark border border-primary/30 rounded-xl p-6 text-center space-y-3 relative overflow-hidden shadow-2xl">
        ...
        <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center mx-auto text-primary">
          {status === 'PASSED' ? (
            <CheckCircle2 className="w-6 h-6 text-trading-up" />
          ) : (
            <ShieldAlert className="w-6 h-6" />
          )}
        </div>

        <h2 className="text-xl font-bold text-on-dark font-sans">
          {status === 'PASSED'
            ? 'Verification Complete'
            : 'Biometric Challenge Required'}
        </h2>
        <p className="text-xs text-muted leading-relaxed">
          {status === 'PASSED' ? (
            <>
              Receiver completed liveness
              {amount ? ` for ₹${amount.toLocaleString('en-IN')}` : ''}. Opening
              UPI PIN pad…
            </>
          ) : (
            <>
        ...
        <div className="bg-canvas border border-hairline-dark rounded-lg p-3 flex items-center justify-between text-xs font-mono">
          ...
          {status === 'PASSED' && (
            <span className="text-trading-up font-bold flex items-center gap-1.5">
              <CheckCircle2 className="w-3.5 h-3.5" /> Verified — Redirecting to PIN…
            </span>
          )}
          {status === 'FAILED' && (
        ...
        {challengeCode && (
          <p className="text-xs font-mono text-on-dark">
            Receiver code: <span className="text-primary font-bold">{challengeCode}</span>
          </p>
        )}

        <p className="text-[10px] font-mono text-muted/80 truncate">
          liveness session: {livenessSessionId}
        </p>
      </div>
      ...
      {status === 'PENDING' && (
        <div className="w-full bg-surface-card-dark border border-hairline-dark rounded-xl p-5 space-y-4 shadow-lg">
          ...
          <p className="text-xs text-muted">
            Share this link with the receiver. After they pass liveness and
            choose whether to share their photo, your PIN pad will unlock.
          </p>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            >
              <ExternalLink className="w-3.5 h-3.5 text-primary" /> Open Receiver
              Page Directly
            </button>
          </div>
        </div>
      ...
      <div className="w-full space-y-3">
        ...
        {status === 'PASSED' && (
          <button
            ...
            className="w-full py-3.5 rounded-md bg-trading-up/90 hover:bg-trading-up text-white font-semibold text-sm shadow-md"
          >
            Continue to UPI PIN
          </button>
        )}
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-surface-card-dark border border-hairline-dark rounded-xl max-w-sm w-full p-6 text-center space-y-5 relative shadow-2xl">
            ...
            <div className="space-y-1">
              <h3 className="text-lg font-bold text-on-dark font-sans">
                Receiver Scan QR
              </h3>
              <p className="text-xs text-muted">
                Scan to open face liveness + consent on the receiver device.
              </p>
            </div>
            ...
            {challengeCode && (
              <p className="text-xs font-mono text-on-dark">
                Code: <span className="text-primary font-bold">{challengeCode}</span>
              </p>
            )}

            <p className="text-[11px] text-muted font-mono leading-tight">
              If the receiver does not complete verification, escrow can refund
              automatically.
            </p>
